...
## version 0.0.2 - cleanup testing, import main function
##################################################
...
#-------------------------------------------------
for-each-channel-setup-global-user-channel-history() { 
  {
...
for-each-channel() { #{ local date_oldest ; date_oldest="${1}" ; local channel_ids ; channel_ids=${@:2} ; { test "${channel_ids}" || { channel_ids="all" ; } ; } ; }
 ...
 { # ${user_channel_history} > temp-user-channel-history
   local user_channel_history
   ${FUNCNAME}-setup-global-user-channel-history 
 }
